...
def vigenere(data, key, mode):
    """
    Performs a vigenere cipher on data,
    given some key, encrypting (1) or
    decrypting (-1) depending on mode.
    ...
    for c in range(0, len(data)):
            if (not data[c].isalpha()): # if the data isn't alphabetic
                    ...
                    index_of_key = string.lowercase.index(stripped_key[(c - num_nonchar) % len(stripped_key)].lower())
                    result += string.lowercase[(index_of_char + (mode * index_of_key)) % 26]
            elif (data[c].isupper()):
                    index_of_char = string.uppercase.index(data[c])
                    index_of_key = string.uppercase.index(stripped_key[(c - num_nonchar) % len(stripped_key)].upper())
...
if __name__ == '__main__':
        ...
        parser = argparse.ArgumentParser(description="encrypt/decrypt vigenere cipher messages")
        parser.add_argument('role', choices=choices, help="whether to encrypt or decrypt")
        parser.add_argument('key', metavar='KEY', type=str, help="the ciper key to encode/decode with")
